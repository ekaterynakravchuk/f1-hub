...
**usePositions.ts:** Two params — `sessionKey: number | undefined, driverNumber: number | undefined`. Both must be defined. Uses `openf1Keys.positions(sessionKey ?? 0, driverNumber ?? 0)` and `sessionKey && driverNumber ? () => fetchPositions(sessionKey, driverNumber) : skipToken`.
  ...
  <verify>
Run `npx tsc --noEmit` — should compile with zero errors. Each hook file should import from openf1 endpoints and query-keys (not jolpica). Verify hook count: `ls src/hooks/use*.ts | wc -l` should show at least 17 (11 existing + 6 new from this plan; useAudioPlayer from Plan 06-03 may also be present if it ran first, making it 18).
  </verify>
  <done>
